...
data = xr.load_dataset(
    os.path.join(root_path, "plate1_stitched_mcam_max_image_dataset.nc"),
    engine="netcdf4",
    mask_and_scale=False,
)
mask = xr.load_dataset(
    os.path.join(root_path, "plate1_stitched_mcam_mask_dataset.nc"),
    engine="netcdf4",
    mask_and_scale=False,
)
...
#
pairs = [(0, 11), (1, 2)]  # plate 1 - water, mitomycin
all_features = []
all_labels = []

for label_index, (a, b) in enumerate(pairs):
    image = data.images[:, [a, b], ...].values.reshape(-1, 3584, 3584, 1)
    image = image[..., 0, np.newaxis]
    image_mask = mask.images[:, [a, b], ...].values.reshape(-1, 3584, 3584, 1)
    ...
    # one label per image in this (a,b) pair across all timepoints
    all_labels += [label_index] * image.shape[0]

# keep dates aligned with features
all_dates = np.tile(np.repeat(np.arange(data.images.shape[0]), 16), len(pairs))

#
root_path = "./"
data = xr.load_dataset(
    os.path.join(root_path, "plate2_stitched_mcam_max_image_dataset.nc"),
    engine="netcdf4",
    mask_and_scale=False,
)
mask = xr.load_dataset(
    os.path.join(root_path, "plate2_stitched_mcam_mask_dataset.nc"),
    engine="netcdf4",
    mask_and_scale=False,
)

#
pairs = [(0, 11), (5, 6)]  # plate 2 - vehicle, chir
...
for label_index, (a, b) in enumerate(pairs):
    ...
    # one label per image in this (a,b) pair across all timepoints
    all_labels += [label_index + 2] * image.shape[0]
...
for label_index, (a, b) in enumerate(pairs):
    ...
    # one label per image in this (a,b) pair across all timepoints
    all_labels += [label_index + 4] * image.shape[0]
...
for day in range(15):
    ...
    for i in range(0 + day * 16, all_features.shape[0], 16 * 15):
    ...
    # 1) Normalize (CLIP usually compared via cosine)
    ...
    # --- color-blind–friendly base colors (Okabe–Ito) ---
